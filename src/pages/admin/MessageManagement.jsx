...
const columns=[
  ...
  {field: 'attachments', headerName: 'Attachments', headerClassName: 'table-header', width: 200, renderCell: (params)=> {
    ...
    attachments.map(i=>{
      ...
      return( <Box>
        <a href={url} download target='_blank' style={{ color: 'black' }}>{RenderContent(file,url)}</a>
      </Box>)
    }):"No Attachments"
  }},
  {field: 'content', headerName: 'Content', headerClassName: 'table-header', width: 400, renderCell: (params)=>{
    return params.row.content ? `${params.row.content}` : "No Message"
  }},
  {field: 'sender', headerName: 'Sent By', headerClassName: 'table-header', width: 200, renderCell: (params)=> <Stack direction={'row'} spacing={'1rem'} alignItems={'center'}>
...
const MessageManagement = () => {
  ...
  useEffect(()=>{
    setRows(dashboardData.messages.map(i=>({...i, id: i._id, sender:{
      name: i.sender.name, avatar: transformImage(i.sender.avatar,50)}, createdAt: moment(i.createdAt).format("MMMM Do YYYY, h:mm:ss a")
    })),[])
  })
  return (
    <AdminLayout>
