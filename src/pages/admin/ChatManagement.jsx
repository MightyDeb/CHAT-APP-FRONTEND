...
const columns=[
  ...
  headerClassName: 'table-header', width: 300},
  ...
  {field:'creator', headerName: 'Created By', headerClassName: 'table-header', width: 150, renderCell: (params)=> <Stack direction='row' alignItems='center' spacing={'1rem'}>
...
const ChatManagement = () => {
  ...
  useEffect(()=>{
    setRows(dashboardData.chats.map(i=>({...i,
      id:i._id, avatar: i.avatar.map(i=> transformImage(i,50)), members: i.members.map(i=> transformImage(i.avatar,50)),
      creator: {
        name: i.creator.name, avatar: transformImage(i.creator.avatar,50)
      }
    })))
  },[])
  return (
